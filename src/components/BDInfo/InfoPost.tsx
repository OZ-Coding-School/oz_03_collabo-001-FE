...
const InfoPost: React.FC<Props> = ({ isQuestion }) => {
  return (
    <div className='flex h-[49px] w-full items-center justify-between'>
      <div>
        <div className='flex flex-nowrap items-center text-xs font-semibold'>
          <p className={twMerge('mr-1 text-primary', !isQuestion && 'hidden')}>
            Q.
          </p>
          ...
          <p
            className={twMerge(
              'w-max-[49px] flex h-[12px] items-center justify-center rounded-[5px] border border-nav p-1 text-[7px] text-nav',
              !isQuestion && 'hidden'
            )}
          >
            답변완료
          ...
          <p
            className={twMerge(
              'flex h-[16px] w-[39px] items-center justify-center bg-[#F6821F30] text-[7px] text-primary',
              isQuestion && 'hidden'
            )}
          >
            질문있어요
